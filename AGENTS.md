...
**角色目录直接放在仓根**，不再套一层桶目录：这样安装到 harness 时无需拍平，仓内相对链接（如 `../pdt-leader/group-conventions.md`）也直接可用。
...
3. **登记一致性**：新增、改名、删除任何角色，下面三处要么都有、要么都没有：本仓 `README.md` 的角色表、`docs/<角色名>.md`、`pdt-leader/skill-inventory.md` 的台账。
