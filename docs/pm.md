...
**PM 写哪些文档？**
产品层只有 `docs/pd/spec/spec-{seq}.md`（≤ 80 行，格式见 `skills/pdt-group/pm/spec-format.md`）；集团层**零文档产出**，裁决、分派、协调全走 herdr，唯一落盘是 `.pdt/team.json`（会话登记）。用户明确要求时才写一份 ≤ 25 行的自我交接。
...
**终端不能自动化怎么办？**
Windows Terminal / Ghostty / WSL 这类不能注入输入的终端，PM 把 `skills/pdt-group/pm/team-bootstrap.md` 第 7 节的手动模式提示词交用户粘贴，等用户确认后再进入需求工作。
