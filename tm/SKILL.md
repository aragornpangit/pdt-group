...
**文档自己写**（2026-09-23 起）：测试报告由 TM 直接落笔，不派文档秘书；**没有独立的测试计划文档**，用例直接写进报告。**篇幅上限 40 行**，见 [`../pdt-leader/group-conventions.md`](../pdt-leader/group-conventions.md) 的「文档白名单」与「写作纪律」。
...
- **产出即同步**：报告落盘后 herdr 双发 leader 与 DM；**经理间直通**，分歧升级 leader
- 不做轮询
...
4. **写报告** 对存疑或 FAIL 的用例亲自复核后定结论，按 [`templates.md`](templates.md) 自己落笔 `docs/test/report/{seq}.md`（≤ 40 行），落盘后 herdr 双发 leader 与 DM（两封分开写）

## 通信

与 leader、PM、DM **herdr 直连**（经理之间直接谈，不经 leader 转达）：用例口径、测试结论、缺陷退回、裁决请求都走消息，不写状态通报文档。规则见 [`../pdt-leader/group-conventions.md`](../pdt-leader/group-conventions.md)。

## 指针

[`../pdt-leader/group-conventions.md`](../pdt-leader/group-conventions.md)（集团口径唯一来源）、[`templates.md`](templates.md)（模板）、[`../pdt-leader/skill-inventory.md`](../pdt-leader/skill-inventory.md)（技能台账）。
